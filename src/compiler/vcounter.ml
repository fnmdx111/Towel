...
type vcounter = VCount of int * (* function count *)
                          int * (* sequence count *)
                          int * (* if count *)
                          int;; (* match count *)
...
let vc_zero = VCount(0, 0, 0, 0);;

let vc_fun_inc c = match c with
  VCount(i, j, k, l) -> VCount(i + 1, j, k, l);;

let vc_seq_inc c = match c with
  VCount(i, j, k, l) -> VCount(i, j + 1, k, l);;

let vc_if_inc c = match c with
  VCount(i, j, k, l) -> VCount(i, j, k + 1, l);;

let vc_match_inc c = match c with
  VCount(i, j, k, l) -> VCount(i, j, k, l + 1);;
...
let vc_to_label = function
    VCount(i, j, k, l) -> Printf.sprintf ":%s%s%s%s"
