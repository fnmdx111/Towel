...
\section{The Towel Compiler (Zihang Chen)}
  ...
  Then it invokes module \texttt{Compile.compile} to transform the AST data structure to the AST of intermediate representation (or AST of bytecode). During this phase, \texttt{Compile} makes use of \texttt{Cseg.(|\mtilde ~\mtilde ~|)} and \texttt{Tasm\_ast} to build the IR AST, it also take advantage of module \texttt{Scoping} to do scope analysis at the same time.
  ...
  \begin{mdframed}[style=detail]
    ...
    The \texttt{Compile.compile} function basically invokes the code generation function \texttt{Compile.g\_word} for each word in the AST. And for each kind of word, \texttt{Compile.g\_word} call all kinds of other code generation functions, for example, for \texttt{Ast.WFunction}, it calls \texttt{Compile.g\_fun}. We didn't import many code optimizations, so all of the \texttt{g\_*} functions are somewhat trivial.
  \end{mdframed}
  ...
  In the last step, the compiler invokes \texttt{Tasm\_bytecode.b\_asm} to get the bytecode representation of the IR.
...
Things are much simpler on the VM-side. The virtual machine frontend \texttt{Tvm} first reads in the bytecode representation of IR, transforms it back into IR AST by invoking \texttt{Tasm\_inv\_bytecode.parse\_bytecode}. Then invokes \texttt{Nvm.exec} to execute all the instructions parsed.
