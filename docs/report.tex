...
Although stack-based programs are hard to reason about, it's really efficient and powerful to use once you get the gist of it. It also enforces programmer's to think about the evaluation order of their potentially side-effected code.
...
\subsection{Object-oriented Programming (not even close)}
...
Thus the two arguments are \texttt{2} and \texttt{0x15}. So we can also reconstruct this instruction to be \texttt{EVAL\_AND\_PUSH(2, 0x15)}. The rest instructions are the same.
