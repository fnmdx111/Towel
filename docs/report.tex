...
\section{The Towel Compiler (Zihang Chen)}
  ...
  \begin{mdframed}[style=detail]
    ...
    The \texttt{Compile.compile} function basically takes a \texttt{Sentence}, then for each word in the \texttt{Sentence}, it invokes \texttt{g\_word}. For each kind of word, \texttt{g\_word} route them to respective \texttt{g\_*} functions. For every kind of word, there exists a \texttt{g\_*} function corresponding it.

Each \texttt{g\_*} function takes a parameter \texttt{ctx} to keep track of the current context of the code generation, for example, the scope environment the piece of AST is in, or if the AST is in a backquote, etc.
